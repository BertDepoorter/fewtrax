"""scipy/bisplev amplitude reference — test-only ground truth.

Evaluates Teukolsky mode amplitudes :math:`A_{\\ell mkn}(a, p, e)` with the
numpy/``scipy.interpolate.bisplev`` path, looping over trajectory points and
modes exactly as FEW's CUDA/CPU amplitude kernel does.  This is the reference
the JAX-native :class:`~fewtrax.amplitude.interp.JAXAmplitudeInterpolator` is
validated against; it lives in ``tests/`` rather than the package because it is
slow, CPU-only, and not JAX-traceable.

(Lifted verbatim from the former ``fewtrax.amplitude.interp.AmplitudeInterpolator``.)
"""
...
from fewtrax.data.loader import _separatrix_numpy


class ScipyAmplitudeReference:
    """numpy/scipy bisplev amplitude evaluator (test reference)."""

    def __init__(self, amp_data):
    ...
    def _eval_block(self, a_abs, p_arr, e_arr) -> np.ndarray:
        ...
        for it in range(N):
            ...
            if self._in_region_A(a_abs, p, e):
    ...
    def evaluate(
        ...
        """Evaluate amplitudes along a trajectory.  Returns (N, n_modes) complex."""
        amps = self._eval_block(
            float(np.abs(a)), np.asarray(p, dtype=float), np.asarray(e, dtype=float)
        )
        if specific_modes is not None:
            amps = amps[:, specific_modes]
        return amps

    def get_mode_power(self, a, p, e, x: float = 1.0) -> np.ndarray:
        """Mean |A|² per mode, shape (n_modes,)."""
        return np.mean(np.abs(self.evaluate(a, p, e, x)) ** 2, axis=0)

    def select_modes(self, a, p, e, threshold: float = 1e-5, x: float = 1.0) -> np.ndarray:
        """Indices of modes with mean power ≥ ``threshold × max``."""
